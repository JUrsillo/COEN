...
//Complexity O(n)
//USess the pivot to determine which values need to be swapped in the array
...
//Swaps the values depending on which value is larger
...
if (low < high) {
	...
	//Calls partition to set index value
	int index = partition (sp, array, low, high);



//Calls quick sort recursively first lowering the high and increasing the low when the first recursive loop breaks
quickSort(sp, array, low, index-1);
quickSort(sp, array, index+1, high);
...
//Complexity O(n)
//Complexity O(n^2): Worst
//Calls the quick sort function and returns the sorted array
void *getElements(SET *sp) {

int i, j;
assert(sp!= NULL);
void** newArray = malloc(sizeof(void*)*sp->count);
assert (newArray != NULL);
for (i = 0, j = 0; i < sp->length; i++){
if (sp->flags[i] == FILLED) {

newArray[j++] = sp->data[i];

}
}
quickSort (sp, newArray, 0, sp->count-1);
return newArray;
}
